...
dat = [{'urls': ['javascript:(function(){window.ALIPAYVIEWAPPEARED=1})();',
       ...
       {'urls': ['https://amap.com'], 'path': 'defpackage/cvj.java'}, {
           'urls': ['www.taobao.com18337258710将账户绑定到淘宝账户上', 'http://github.com/s12345678quare/leakcanary',
                    'http://www.amap.com',
...
def main(types: str, datas: list):
    ...
    if types == "urls":
        ...
        for data in first_result:  # 遍历当前的全部数据源,
            data_dict, need_datas = {}, []
            # 如果response中解析到数据,,需要拼装
            for task in resp_result:
                res = AnalysisUrlsEmails.extract_phone_card_passport(task['resp'])  # re匹配响应内容(json/html)
                if res:
                    res['url'] = task['url']
                    # res['response'] = task['resp']
