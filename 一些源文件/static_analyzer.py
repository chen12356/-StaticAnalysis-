...
def key(data, key_name):
    """Return the data for a key_name.返回秘钥名称数据"""
...
def static_analyzer(request, api=False):
    ...
    try:
        if api:
            ...
            typ = request.GET['type'] # 类型：apk、zip等
            checksum = request.GET['checksum'] # md5值
            filename = request.GET['name']  #文件名
            rescan = str(request.GET.get('rescan', 0)) # 重新扫描次数
        # Input validation
        app_dic = {}
        ...
        if (
            ...
            if typ == 'apk':
                ...
                if db_entry.exists() and rescan == '0':
                    #二次查询直接从数据库取---> 新增 解析的数据：需要查询新的数据库
                    context = get_context_from_db_entry(db_entry)
                else: #新建
                    app_dic['app_file'] = app_dic[
                        'md5'] + '.apk'  # 新的文件名：参数MD5与参数旧文件名的后缀结合
                    ...
                    app_dic['files'] = unzip(
                        app_dic['app_path'], app_dic['app_dir']) #解压，得到目标文件

                    if not app_dic['files']:
                        # Can't Analyze APK, bail out.
                        msg = 'APK file is invalid or corrupt'  #APK文件无效或损坏
                        if api:
                    ...
                    cert_dic = cert_info(
                        ...
                        app_dic['app_file'])
                    print('----------------------')
                    print(app_dic['app_dir'])
                    print(app_dic['app_path'])
                    print(app_dic['app_name'])
                    print('=======================')
                ...
                print('需要代码分析结果：{0}'.format(context['code_analysis']))
                print('=========================')
